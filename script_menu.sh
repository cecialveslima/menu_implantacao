...
realiza_implantacao() {
 	...
 	#Usar somente se for produção que gera uma pasta por GMUD.
        #echo "Hoje é  dia: $DATA_ATUAL A pasta da GMUD deve conter a mesma data. "

        # Verifica se foi passado o container e o local (tomcat6 ou tomcat9)
        if [ -z "$1" ] || [ -z "$2" ]; then
                echo "ATENCAO: ./script_menu.sh <nome_do_container> <tomcat6 OU tomcat9>"
                exit 1
        fi
        ...
        BACKUP_DIR="/home/dev-SEUNOME/containers/backup_container/"
        #LOCAL_CLASSES="/home/dev-SEUNOME/GMUD${ANO}/${DATA_ATUAL}/${CONTAINER_NAME}/" #SOMENTE PARA producao
        LOCAL_CLASSES="/home/dev-SEUNOME/GMUD${ANO}/${CONTAINER_NAME}/"
        ...
        if [ -e "$LOCAL_CLASSES" ]; then
                ...
                if [ -e "$CONTAINER_NAME" ]; then
                        ...
                        echo "Iniciando o processo de BACKUP da pasta: '${CONTAINER_NAME}'"
...
while true; do
        ...
        8)
                echo "Para Serviço Tomcat6, Tomcat9"
                para_tomcats
                ;;
        9)
                echo "Inicia Serviços Tomcat6, Tomcat9"
